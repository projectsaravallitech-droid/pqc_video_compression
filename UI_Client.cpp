...
void drawUI(Mat& ui, bool serverMode, Mat& videoFrame) {
    ...
    putText(ui, "Client UI", Point(r_mode.x + 20, r_mode.y + 55), FONT_HERSHEY_SIMPLEX, 0.7, Scalar(255,255,255), 1);
    rectangle(ui, b_start, Scalar(200,200,200), 1);
    putText(ui, streamToClient ? "Stop" : "Connect", Point(b_start.x + 10, b_start.y + 25), FONT_HERSHEY_SIMPLEX, 0.6, Scalar(255,255,255), 1);
    ...
    putText(ui, "Record", Point(b_rec.x + 10, b_rec.y + 25), FONT_HERSHEY_SIMPLEX, 0.6, Scalar(255,255,255), 1);
    ...
    rectangle(ui,
              ...
              Scalar(20, 20, 20),
              FILLED);
    putText(ui, fpsText, fpsOrigin, FONT_HERSHEY_SIMPLEX, 0.6, Scalar(0,255,255), 2);
    putText(ui, "Status: " + appStatus + (RxCount > 0 ? " | Frames Rx: " + to_string(RxCount) : ""), Point(r_video.x + 10, r_video.y + r_video.height - 15), FONT_HERSHEY_SIMPLEX, 0.6, Scalar(0,255,255), 2);
...
void onMouse(int event, int x, int y, int, void*) {
    if (event == EVENT_LBUTTONDOWN) {
        if (isBtn(x, y, b_gen)) {
            appStatus = "Guided Mode active! The Server will auto-ping handshake keys here.";
        }
        else if (isBtn(x, y, b_stream) || isBtn(x, y, b_start)) {
            streamToClient = true;
            appStatus = "Live decoder unlocked! Awaiting packets...";
        }
    }
}
